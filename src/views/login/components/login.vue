...
import { ref, reactive, nextTick } from "vue"; // 引入 Vue 基础函数
...
import { ElForm } from "element-plus"; // 引入表单组件
import { debounce } from "lodash-es"; // 引入防抖函数

type LoginMethod = "email" | "phone"; // 定义登录方式的类型
...
// 登录逻辑
const doLogin = async () => {
  if (!formRef.value) return; // 如果表单不存在，则返回
  try {
