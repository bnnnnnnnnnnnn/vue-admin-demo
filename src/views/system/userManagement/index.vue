...
import type { Role, User } from '@/api/system/type'
import {
  createUser,
  deleteUser,
  getRoles,
...
// 打开编辑弹窗（修改角色 / 修改密码）
function openEditUserDialog(user: User, type: 'isEditMode' | 'isEditPassword') {
...
<template>
  <el-card>
    ...
    <el-table v-loading="loading" :data="users" border style="margin-top: 20px">
      ...
      <el-table-column prop="role" label="角色" align="center">
        <template #default="{ row }">
          <el-tag v-for="r in row.role" :key="r.id" class="mr-1">
            {{ r.name }}
          </el-tag>
        </template>
      </el-table-column>
  ...
  >
    <el-form label-width="80px">
      <!-- 注册方式 -->
      <el-form-item v-if="!isEditMode && !isEditPassword" label="注册方式">
        <el-select
      ...
      <!-- 邮箱 -->
      <el-form-item v-if="registerMethod === 'email'" label="邮箱">
        <el-input
      ...
      <!-- 手机号 -->
      <el-form-item v-if="registerMethod === 'phone'" label="手机号">
        <el-input
      ...
      <!-- 角色 -->
      <el-form-item v-if="!isEditPassword" label="角色">
        <el-select
      ...
      <el-form-item v-if="!isEditPassword" label="用户详情">
        <el-input
