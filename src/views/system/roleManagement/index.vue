...
const currentRole = ref<{ id?: number | null; name: string; description: string }>({
  id: null,
  name: "",
  description: ""
});
...
const deleteRole = async (role:any) => {
  ...
  await supabase.from("role_permissions").delete().match({ role_id: role.id }); // 删除角色的权限关联
  fetchRoles();
};
...
const saveRole = async () => {
  ...
  try {
    ...
    // 2. 更新角色的权限
    await supabase.from("role_permissions").delete().match({ role_id: roleId }); // 先清除旧权限
